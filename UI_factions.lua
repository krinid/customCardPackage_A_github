...
--	+ prefHeight	[number]			How tall the element prefers to be
...
--	+ flexHeight	[number]			A number from 0 to 1 indicating how much of the remaining space this element wishes to take up (vertical)

function newLabel(name, parent, text, color, prefWidth, prefHeight, flexWidth, flexHeight)
	local lab = label(getParent(parent), text, color, prefWidth, prefHeight, flexWidth, flexHeight);			-- Create the label
...
function newButton(name, parent, text, onClick, color, interactable, prefWidth, prefHeight, flexWidth, flexHeight)
	local but = button(getParent(parent), text, onClick, color, interactable, prefWidth, prefHeight, flexWidth, flexHeight);		-- Create the button
...
function newCheckbox(name, parent, text, isChecked, interactable, onValueChanged, prefWidth, prefHeight, flexWidth, flexHeight)
	local box = checkbox(getParent(parent), text, isChecked, interactable, onValueChanged, prefWidth, prefHeight, flexWidth, flexHeight);		-- Create the checkbox
...
function newTextField(name, parent, placeholderText, text, characterLimit, interactable, prefWidth, prefHeight, flexWidth, flexHeight)
	local tif = textInputField(getParent(parent), placeholderText, text, characterLimit, interactable, prefWidth, prefHeight, flexWidth, flexHeight);
...
function newNumberField(name, parent, sliderMin, sliderMax, value, interactable, wholeNumbers, boxPreferredWidth, sliderPreferredWidth, prefWidth, prefHeight, flexWidth, flexHeight)
	local nif = numberInputField(getParent(parent), sliderMin, sliderMax, value, interactable, wholeNumbers, boxPreferredWidth, sliderPreferredWidth, prefWidth, prefHeight, flexWidth, flexHeight);
...
--	+ color			[string]			Color string in the format #HHHHHH (H = hexadecimal) or one of the build in colors (Blue, Green, Aqua, etc)
--	+ prefWidth		[number]			How wide the element prefers to be
--	+ flexHeight	[number]			How tall the element prefers to be
--	+ flexWidth		[number]			A number from 0 to 1 indicating how much of the remaining space this element wishes to take up (horizontal)
--	+ flexHeight	[number]			A number from 0 to 1 indicating how much of the remaining space this element wishes to take up (vertical)

function label(parent, text, color, prefWidth, prefHeight, flexWidth, flexHeight)
	prefWidth = prefWidth or -1; prefHeight = prefHeight or -1; flexWidth = flexWidth or 0; flexHeight = flexHeight or 0;			-- Set optional paramaters to default value if not specified
	return UI.CreateLabel(parent).SetText(text).SetColor(getColorFromString(color)).SetPreferredWidth(prefWidth).SetPreferredHeight(prefHeight).SetFlexibleWidth(flexWidth).SetFlexibleHeight(flexHeight);
end
...
function button(parent, text, onClick, color, interactable, prefWidth, prefHeight, flexWidth, flexHeight)
	prefWidth = prefWidth or -1; prefHeight = prefHeight or -1; flexWidth = flexWidth or 0; flexHeight = flexHeight or 0; if interactable == nil then interactable = true; end;		-- Set optional paramaters to default value if not specified
	local but = UI.CreateButton(parent).SetText(text).SetColor(getColorFromString(color)).SetOnClick(onClick).SetInteractable(interactable).SetPreferredWidth(prefWidth).SetPreferredHeight(prefHeight).SetFlexibleWidth(flexWidth).SetFlexibleHeight(flexHeight);
...
function checkbox(parent, text, isChecked, interactable, onValueChange, prefWidth, prefHeight, flexWidth, flexHeight)
	prefWidth = prefWidth or -1; prefHeight = prefHeight or -1; flexWidth = flexWidth or 0; flexHeight = flexHeight or 0; if interactable == nil then interactable = true; end; if isChecked == nil then isChecked = true; end; onValueChange = onValueChange or function() end;			-- Set optional paramaters to default value if not specified
	local box = UI.CreateCheckBox(parent).SetText(text).SetIsChecked(isChecked).SetOnValueChanged(onValueChange).SetInteractable(interactable).SetPreferredWidth(prefWidth).SetPreferredHeight(prefHeight).SetFlexibleWidth(flexWidth).SetFlexibleHeight(flexHeight);
...
function textInputField(parent, placeholderText, text, characterLimit, interactable, prefWidth, prefHeight, flexWidth, flexHeight)
	prefWidth = prefWidth or -1; prefHeight = prefHeight or -1; flexWidth = flexWidth or 0; flexHeight = flexHeight or 0; if interactable == nil then interactable = true; end; text = text or ""; characterLimit = characterLimit or 0;
	return UI.CreateTextInputField(parent).SetText(text).SetPlaceholderText(placeholderText).SetCharacterLimit(characterLimit).SetInteractable(interactable).SetPreferredWidth(prefWidth).SetPreferredHeight(prefHeight).SetFlexibleWidth(flexWidth).SetFlexibleHeight(flexHeight);
end
...
function numberInputField(parent, sliderMin, sliderMax, value, interactable, wholeNumbers, boxPreferredWidth, sliderPreferredWidth, prefWidth, prefHeight, flexWidth, flexHeight)
	prefWidth = prefWidth or -1; prefHeight = prefHeight or -1; flexWidth = flexWidth or 0; flexHeight = flexHeight or 0; if interactable == nil then interactable = true; end; if wholeNumbers == nil then wholeNumbers = true; end; boxPreferredWidth = boxPreferredWidth or -1; sliderPreferredWidth = sliderPreferredWidth or -1;
	return UI.CreateNumberInputField(parent).SetWholeNumbers(wholeNumbers).SetSliderMinValue(sliderMin).SetSliderMaxValue(sliderMax).SetValue(value).SetInteractable(interactable).SetBoxPreferredWidth(boxPreferredWidth).SetSliderPreferredWidth(sliderPreferredWidth).SetPreferredWidth(prefWidth).SetPreferredHeight(prefHeight).SetFlexibleWidth(flexWidth).SetFlexibleHeight(flexHeight);
end
...
function getObject(name)
	...
	--print ("getObject - Name: " .. tostring(name).."::");
