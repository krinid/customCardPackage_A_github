-- original dabo1 code
function Client_PresentConfigureUI_originalDabo(rootParent)
	root=rootParent;
	...
	PestCardCheckbox=UI.CreateCheckBox(horzlist[0]).SetText('Pestilence card').SetIsChecked(PestCardIninit).SetOnValueChanged(IncludeExcludePestilenzCard);
	...
	NukeCardCheckbox=UI.CreateCheckBox(horzlist[4]).SetText('Nuke card').SetIsChecked(NukeCardIninit).SetOnValueChanged(IncludeExcludeNukeCard);
	...
	if(Mod.Settings.IsolationEnabled ~= nil)then
   		IsolationCardIninit=Mod.Settings.IsolationEnabled;
	end
	IsolationCardDurationinit=1;
	if(Mod.Settings.IsolationDuration ~= nil)then
   		IsolationCardDurationinit=Mod.Settings.IsolationDuration;
	end
	IsolationCardPiecesNeededinit=4;
	if(Mod.Settings.IsolationPiecesNeeded ~= nil)then
   		IsolationCardPiecesNeededinit=Mod.Settings.IsolationPiecesNeeded;
	end
	IsolationCardStartPiecesinit=1;
	if(Mod.Settings.IsolationStartPieces ~= nil)then
   		IsolationCardStartPiecesinit=Mod.Settings.IsolationStartPieces;
	end
	horzlist[12] = UI.CreateHorizontalLayoutGroup(rootParent);
	IsolationCardCheckbox=UI.CreateCheckBox(horzlist[12]).SetText('Isolation card').SetIsChecked(IsolationCardIninit).SetOnValueChanged(IncludeExcludeIsolationCard);
